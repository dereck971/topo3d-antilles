...
let polyMode=false,polyVertices=[],polyGuide=null;
...
const ileConfig={
    ...
    guyane:{center:[-53.20,4.00],zoom:7,pitch:40,bearing:0},
    reunion:{center:[55.53,-21.12],zoom:10.5,pitch:50,bearing:-10}
};
const ileName={guadeloupe:'Guadeloupe',martinique:'Martinique',guyane:'Guyane',reunion:'La Réunion'};
const ileDept={guadeloupe:'971',martinique:'972',guyane:'973',reunion:'974'};
...
// ─── LAYER STATE ───
const layerState={'cadastre-wms':true,'buildings-3d':false,'bdtopo-bat':false,'ortho-ign':false,'hillshade':false,'contours':false,'hydro':false,'zones-risk':false,'plu-zonage':false,'plu-prescrip':false,'natura2000':false,'znieff1':false,'littoral':false,'monuments-h':false,'lidar-mnt':false,'lidar-mns':false,'lidar-mnh':false};
...
function addParcelleSource(){
    ...
    // Polygon libre sources
    if(!map.getSource('draw-poly')){
        const emptyFC={type:'FeatureCollection',features:[]};
        map.addSource('draw-poly',{type:'geojson',data:emptyFC});
        map.addLayer({id:'draw-poly-fill',type:'fill',source:'draw-poly',paint:{'fill-color':'#ffdd00','fill-opacity':0.2}});
        map.addLayer({id:'draw-poly-line',type:'line',source:'draw-poly',paint:{'line-color':'#ffdd00','line-width':2,'line-dasharray':[2,1]}});
        map.addSource('draw-verts',{type:'geojson',data:emptyFC});
        map.addLayer({id:'draw-verts-circle',type:'circle',source:'draw-verts',paint:{'circle-radius':5,'circle-color':'#ffdd00','circle-stroke-width':2,'circle-stroke-color':'#fff'}});
        map.addSource('draw-guide',{type:'geojson',data:emptyFC});
        map.addLayer({id:'draw-guide-line',type:'line',source:'draw-guide',paint:{'line-color':'#ffdd00','line-width':1,'line-dasharray':[4,3]}});
    }
}
...
function addCustomLayers(){
    ...
    if(!map.getSource('lidar-mnt-src')){
        map.addSource('lidar-mnt-src',{type:'raster',tiles:['https://data.geopf.fr/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=IGNF_LIDAR-HD_MNT_ELEVATION.ELEVATIONGRIDCOVERAGE.SHADOW&TILEMATRIXSET=PM&TILEMATRIX={z}&TILEROW={y}&TILECOL={x}&FORMAT=image/png&STYLE=normal'],tileSize:256,minzoom:10,maxzoom:18});
    ...
    if(!map.getSource('lidar-mns-src')){
        map.addSource('lidar-mns-src',{type:'raster',tiles:['https://data.geopf.fr/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=IGNF_LIDAR-HD_MNS_ELEVATION.ELEVATIONGRIDCOVERAGE.SHADOW&TILEMATRIXSET=PM&TILEMATRIX={z}&TILEROW={y}&TILECOL={x}&FORMAT=image/png&STYLE=normal'],tileSize:256,minzoom:10,maxzoom:18});
    ...
    if(!map.getSource('lidar-mnh-src')){
        map.addSource('lidar-mnh-src',{type:'raster',tiles:['https://data.geopf.fr/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=IGNF_LIDAR-HD_MNH_ELEVATION.ELEVATIONGRIDCOVERAGE.SHADOW&TILEMATRIXSET=PM&TILEMATRIX={z}&TILEROW={y}&TILECOL={x}&FORMAT=image/png&STYLE=normal'],tileSize:256,minzoom:10,maxzoom:18});
    ...
    // BD TOPO IGN — bâtiments avec hauteurs réelles (photogrammétrie)
    if(!map.getSource('bdtopo-bat-src')){
        map.addSource('bdtopo-bat-src',{type:'geojson',data:{type:'FeatureCollection',features:[]}});
        map.addLayer({
            id:'bdtopo-bat',
            type:'fill-extrusion',
            source:'bdtopo-bat-src',
            minzoom:14,
            layout:{visibility:'none'},
            paint:{
                'fill-extrusion-color':[
                    'interpolate',['linear'],['coalesce',['get','hauteur'],6],
                    0,'#b8d4e3',
                    5,'#90b4ce',
                    10,'#6894b9',
                    20,'#4a7a9e',
                ...
                'fill-extrusion-height':['coalesce',['get','hauteur'],6],
...
// ─── BD TOPO WFS LOADER — bâtiments IGN hauteurs réelles ───
let bdtopoLoadCtrl=null,bdtopoDebounce=null;
async function loadBDTopo(){
    const zoom=map.getZoom();
    const src=map.getSource('bdtopo-bat-src');
    if(!src) return;
    if(zoom<14){src.setData({type:'FeatureCollection',features:[]});return;}
    const b=map.getBounds();
    const bbox=`${b.getWest()},${b.getSouth()},${b.getEast()},${b.getNorth()}`;
    const url=`https://data.geopf.fr/wfs/ows?SERVICE=WFS&VERSION=2.0.0&REQUEST=GetFeature&TYPENAMES=BDTOPO_V3:batiment&BBOX=${bbox},EPSG:4326&OUTPUTFORMAT=application/json&COUNT=2000`;
    try{
        if(bdtopoLoadCtrl) bdtopoLoadCtrl.abort();
        bdtopoLoadCtrl=new AbortController();
        const r=await fetch(url,{signal:bdtopoLoadCtrl.signal});
        if(r.ok){const d=await r.json();src.setData(d);}
    }catch(e){if(e.name!=='AbortError') console.warn('BD TOPO WFS:',e);}
}
function scheduleBDTopo(){clearTimeout(bdtopoDebounce);bdtopoDebounce=setTimeout(loadBDTopo,400);}
...
function toggleLayer(el,layerId){
    ...
    // BD TOPO bâtiments — charger/décharger en sync avec buildings-3d
    if(layerId==='buildings-3d'){
        if(map.getLayer('bdtopo-bat')) map.setLayoutProperty('bdtopo-bat','visibility',on?'visible':'none');
        if(on){loadBDTopo();map.on('moveend',scheduleBDTopo);}
        else{map.off('moveend',scheduleBDTopo);if(bdtopoLoadCtrl) bdtopoLoadCtrl.abort();}
    }
}
...
function switchIle(name){
    ...
    document.getElementById('ile-label').textContent=ileName[name]||'Guadeloupe';
    ['gp','mq','gf','re'].forEach(code=>{
        const el=document.getElementById('btn-'+code);
        if(el) el.classList.toggle('active',name==={gp:'guadeloupe',mq:'martinique',gf:'guyane',re:'reunion'}[code]);
    });
...
function updateCommuneDropdown(ile){
    ...
    const allCommunes={
        ...
        guyane:{
            '97301':'Cayenne','97302':'Kourou','97303':'Macouria','97304':'Matoury','97305':'Montsinéry-Tonnegrande',
            '97306':'Rémire-Montjoly','97307':'Roura','97308':'Saint-Georges','97309':'Saint-Laurent-du-Maroni',
            '97310':'Sinnamary','97311':'Apatou','97312':'Awala-Yalimapo','97313':'Camopi','97314':'Grand-Santi',
            '97315':'Iracoubo','97316':'Mana','97317':'Maripasoula','97318':'Ouanary','97319':'Papaïchton',
            '97320':'Régina','97321':'Saül','97322':'Saint-Élie'
        },
        reunion:{
            '97401':'Les Avirons','97402':'Bras-Panon','97403':'Cilaos','97404':'Entre-Deux','97405':'L\'Étang-Salé',
            '97406':'Petite-Île','97407':'La Plaine-des-Palmistes','97408':'Le Port','97409':'La Possession',
            '97410':'Saint-André','97411':'Saint-Benoît','97412':'Saint-Denis','97413':'Saint-Joseph',
            '97414':'Saint-Leu','97415':'Saint-Louis','97416':'Saint-Paul','97417':'Saint-Philippe',
            '97418':'Saint-Pierre','97419':'Sainte-Marie','97420':'Sainte-Rose','97421':'Sainte-Suzanne',
            '97422':'Salazie','97423':'Le Tampon','97424':'Les Trois-Bassins'
        }
    };
    const communes=allCommunes[ile]||allCommunes.guadeloupe;
...
document.getElementById('ile-label').textContent=ileName[ile]||'Guadeloupe';
['gp','mq','gf','re'].forEach(code=>{
    const el=document.getElementById('btn-'+code);
    if(el) el.classList.toggle('active',ile==={gp:'guadeloupe',mq:'martinique',gf:'guyane',re:'reunion'}[code]);
});
...
map.on('click',async e=>{
    if(polyMode){handlePolyClick(e);return}
...
map.on('mousemove',e=>{
    ...
    // Polygon guide line
    if(polyMode&&polyVertices.length>0){
        const last=polyVertices[polyVertices.length-1];
        const{lng,lat}=e.lngLat;
        map.getSource('draw-guide')?.setData({type:'FeatureCollection',features:[{type:'Feature',geometry:{type:'LineString',coordinates:[last,[lng,lat]]}}]});
    }
});

map.on('dblclick',e=>{
    if(polyMode&&polyVertices.length>=3){
        e.preventDefault();
        closePolygon();
    }
});
...
function togglePolyMode(){
    ...
    drawMode=false;
    polyVertices=[];
    document.body.style.cursor=polyMode?'crosshair':'auto';
    const emptyFC={type:'FeatureCollection',features:[]};
    map.getSource('draw-poly')?.setData(emptyFC);
    map.getSource('draw-verts')?.setData(emptyFC);
    map.getSource('draw-guide')?.setData(emptyFC);
    const hint=document.getElementById('mapHint');
    if(polyMode){
        hint.classList.remove('hidden');
        hint.textContent='Cliquez pour ajouter des sommets. Double-cliquez pour fermer le polygone. Echap pour annuler.';
    }else{
...
function handlePolyClick(e){
    const{lng,lat}=e.lngLat;
    polyVertices.push([lng,lat]);
    // Update vertices display
    map.getSource('draw-verts')?.setData({type:'FeatureCollection',features:polyVertices.map(v=>({type:'Feature',geometry:{type:'Point',coordinates:v}}))});
    // Update polygon preview (partial)
    if(polyVertices.length>=2){
        const coords=[...polyVertices,polyVertices[0]];
        map.getSource('draw-poly')?.setData({type:'FeatureCollection',features:[{type:'Feature',geometry:{type:'Polygon',coordinates:[coords]}}]});
    }
}
...
function closePolygon(){
    ...
    const coords=[...polyVertices,polyVertices[0]];
    map.getSource('draw-poly')?.setData({type:'FeatureCollection',features:[{type:'Feature',geometry:{type:'Polygon',coordinates:[coords]}}]});
    map.getSource('draw-guide')?.setData({type:'FeatureCollection',features:[]});
    const areaM2=calcPolyArea(polyVertices);
    drawnArea=areaM2;
    const price=calcZonePrice(areaM2);
    showZoneResult(areaM2,price);
    polyMode=false;
...
function calcPolyArea(verts){
    const latAvg=verts.reduce((s,v)=>s+v[1],0)/verts.length;
    const mLng=111320*Math.cos(latAvg*Math.PI/180);
    const mLat=110540;
    let area=0;
    for(let i=0;i<verts.length;i++){
        const j=(i+1)%verts.length;
        area+=(verts[i][0]*mLng)*(verts[j][1]*mLat);
        area-=(verts[j][0]*mLng)*(verts[i][1]*mLat);
    }
...
function showZoneResult(areaM2,price){
    ...
    document.getElementById('c-com').textContent=ileName[currentIle]||'Guadeloupe';
    ...
    document.getElementById('r-com').textContent=ileName[currentIle]||'Guadeloupe';
...
function showParc(f){
    ...
    document.getElementById('r-ref').textContent=`${ileDept[currentIle]||'971'} ${ref}`;
...
function selOffer(el,o){
    ...
    const pr={parcelle:'19€',light:'29€',essentiel:'59€',complet:'129€',premium:'249€',pro:'349€/mois'};
...
Object.assign(window, {
    ...
    toggleDrawMode, togglePolyMode, openTopoPlan, closeTopoPlan,
