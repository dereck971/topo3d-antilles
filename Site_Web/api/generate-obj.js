...
export default async function handler(req, res) {
  ...
  try {
    const { geometry, elevation, resolution: reqRes, tier } = req.body;
    // LiDAR HD: 1m for complet/premium tiers, 5m otherwise
    const hdTiers = ['complet', 'premium', 'pro'];
    const resolution = reqRes || (hdTiers.includes(tier) ? '1m' : '5m');
...
function generateOBJMesh(geometry, elevation, resolution) {
  ...
  obj += `# Source: IGN RGE ALTI / LiDAR HD (${resolution === '1m' ? 'haute definition' : 'standard'})\n\n`;
