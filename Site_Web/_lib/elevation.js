 ...
 * @param {boolean} hd - LiDAR HD mode: 1m grid for premium exports (default false)
...
export async function fetchElevationGrid(bbox, resolution = 5, hd = false) {
  ...
  console.log(`[elevation] Grid ${nCols}x${nRows} = ${totalPoints} points, resolution ${resolution}m${hd ? ' (LiDAR HD)' : ''}`);

  // Safety: cap at 100k points to avoid IGN API overload (especially at 1m HD)
  if (totalPoints > 100000) {
    const scale = Math.sqrt(totalPoints / 100000);
    resolution = Math.ceil(resolution * scale);
    console.warn(`[elevation] Grid too large, falling back to ${resolution}m resolution`);
    return fetchElevationGrid(bbox, resolution, false);
  }
