...
description: >
  ...
  "modèle numérique de terrain", "courbes de niveau 3D", "export topographie", "fichier 3D terrain", "LiDAR", "données LiDAR",
  "télécharger LiDAR", "nuage de points", ou toute demande liée à la génération de données topographiques exploitables en CAO/BIM
  (SketchUp, AutoCAD, Revit, Blender). Couvre la Guadeloupe (971) et potentiellement toute la France via les APIs IGN.
  Même si l'utilisateur ne dit pas explicitement "3D" mais parle d'un terrain, d'une parcelle à modéliser,
  de données altimétriques à exploiter, ou de données LiDAR à télécharger, ce skill est le bon choix.
---
...
## Sources de données LiDAR HD — Téléchargement haute résolution

En complément de l'API altimétrie IGN (RGE ALTI) utilisée par le script pour générer les terrains 3D, les données brutes LiDAR HD de l'IGN sont disponibles en téléchargement gratuit pour la Guadeloupe (acquisitions terminées par Sintegra).

### Liens de téléchargement (Géoplateforme — cartes.gouv.fr)

| Produit | Description | Lien |
|---|---|---|
| **Nuages de points LiDAR HD** | Fichiers .LAZ 1.4 (≥10 pts/m², dalles 1km×1km, 11 classes ASPRS) | https://cartes.gouv.fr/telechargement/IGNF_NUAGES-DE-POINTS-LIDAR-HD |
| **MNT LiDAR HD** | Modèle Numérique de Terrain (sol nu, grille raster) | https://cartes.gouv.fr/telechargement/IGNF_MNT-LIDAR-HD |
| **MNS LiDAR HD** | Modèle Numérique de Surface (sol + sursol : bâtiments, végétation) | https://cartes.gouv.fr/telechargement/IGNF_MNS-LIDAR-HD |
| **MNH LiDAR HD** | Modèle Numérique de Hauteur (hauteur du sursol = MNS - MNT) | https://cartes.gouv.fr/telechargement/IGNF_MNH-LIDAR-HD |

### Autres points d'accès

- **Géoservices IGN** : https://geoservices.ign.fr/lidarhd (documentation complète, visualisation, API WFS/WMS)
- **Data.gouv.fr** : https://www.data.gouv.fr/datasets/nuages-de-points-lidar-hd (jeu de données open data)
- **Guide d'exploitation OFB** : https://ofb.hal.science/hal-04702345v4 (tutoriel complet pour exploiter les données LiDAR HD)
- **DEAL Guadeloupe** : https://www.guadeloupe.developpement-durable.gouv.fr/donnees-publiques-ign-en-acces-libre-a3448.html

### Caractéristiques techniques du LiDAR HD

- **Licence** : Etalab 2.0 (gratuit, réutilisation commerciale autorisée)
- **Format** : LAZ 1.4 (dalles de 1km × 1km)
- **Densité** : ≥ 10 points/m² en moyenne
- **Classification ASPRS** : 11 catégories — sol, végétation basse, végétation moyenne, végétation haute, bâtiment, eau, tablier de pont, sursol pérenne, points virtuels, divers bâtis, non classé
- **Précision altimétrique** : ±0.2m
- **Couverture Guadeloupe** : Acquisitions terminées (opérateur : Sintegra)
- **Programme national** : > 400 000 km² couverts à l'échelle France

### Quand utiliser LiDAR HD vs API altimétrie ?

| Critère | API altimétrie (RGE ALTI) | LiDAR HD brut |
|---|---|---|
| **Usage** | Génération automatique terrain 3D (ce skill) | Études approfondies, BIM détaillé, géomètres |
| **Résolution** | 1-5m | ≥10 pts/m² (~0.3m) |
| **Accès** | API temps réel, intégré au script | Téléchargement de dalles .LAZ |
| **Vitesse** | Immédiat (quelques secondes) | Téléchargement puis traitement local |
| **Poids données** | Léger (requêtes API) | Lourd (centaines de Mo par dalle) |
| **Logiciels** | SketchUp, Blender (via OBJ produit) | CloudCompare, QGIS, LAStools, Revit, Global Mapper |
| **Recommandation** | Pré-études, faisabilité, visualisation rapide | Projets de construction, topographie de précision, permis de construire |

→ **Si l'utilisateur demande des données LiDAR brutes** (nuages de points, fichiers .LAZ, .LAS) ou des données de très haute précision, lui fournir les liens de téléchargement ci-dessus et recommander CloudCompare (gratuit) ou QGIS pour les visualiser.

→ **Pour une pré-étude rapide**, utiliser le script de ce skill qui s'appuie sur l'API RGE ALTI — suffisant pour les analyses de faisabilité et les visualisations 3D.
...
**Utilisateur :** "Je cherche les données LiDAR de la Guadeloupe"
→ Fournir les liens de téléchargement LiDAR HD (cartes.gouv.fr)
→ Expliquer les 4 produits disponibles (nuage de points, MNT, MNS, MNH)
→ Recommander CloudCompare ou QGIS pour exploiter les fichiers .LAZ
→ Proposer de générer un terrain 3D simplifié via le script si l'utilisateur a une parcelle précise
